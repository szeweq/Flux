...
public abstract class AbstractMachineTile extends LockableTileEntity implements IEnergyReceiver, ISidedInventory, IInventoryIO, IRecipeHolder, IRecipeHelperPopulator, ITickableTileEntity {
			...
			if (isPowered() && !inputEmpty) {
				...
				if (recipe != null && canProcess(recipe)) {
					...
					if (process++ == processTotal) {
						process = 0;
						processTotal = recipe.getProcessTime();
	...
	protected boolean canProcess(AbstractMachineRecipe recipe) {
		if (recipe != null) {
	...
	protected void produceResult(AbstractMachineRecipe recipe) {
		if (recipe != null && canProcess(recipe)) {
			ItemStack result = recipe.getRecipeOutput();
	...
	public void setInventorySlotContents(int index, ItemStack stack) {
		...
		if (index < inputSize && !same) {
			processTotal = world.getRecipeManager().getRecipe(recipeType, this, world).map(AbstractMachineRecipe::getProcessTime).orElse(200);
			process = 0;
			markDirty();
		}
	}
	...
	public void updateRecipes(PlayerEntity player) {
		...
		List<IRecipe<?>> recipes = recipesCount.object2IntEntrySet().stream()
				.map(e -> {
					...
					if (recipe != null) {
						collectExperience(player, e.getIntValue(), ((AbstractMachineRecipe) recipe).getExperience());
					}
					return recipe;
				})
