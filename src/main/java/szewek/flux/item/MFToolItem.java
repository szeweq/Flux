...
public class MFToolItem extends Item {
	public MFToolItem(Properties properties) {
		super(properties);
	}
	...
	public ActionResultType onItemUse(ItemUseContext itemUse) {
		...
		if (!w.isRemote && p != null) {
			...
			if (tile != null) {
				...
				lazy.ifPresent(ie -> p.sendMessage(
						new TranslationTextComponent("flux.energystat", ie.getEnergyStored(), ie.getMaxEnergyStored())
				));
			}
